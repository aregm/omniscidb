...
AbstractBuffer* FileMgr::createBuffer(const MemoryLevel memLevel, const ChunkKey& key,
...
AbstractBuffer* FileMgr::getBuffer(const MemoryLevel memLevel, const ChunkKey& key, const size_t numBytes) {
  mapd_shared_lock<mapd_shared_mutex> chunkIndexReadLock(chunkIndexMutex_);
  auto chunkIt = chunkIndex_.find(key);
  if (chunkIt == chunkIndex_.end()) {
    LOG(FATAL) << "Chunk does not exist for key: " << showChunk(key);
...
void FileMgr::fetchBuffer(const MemoryLevel memLevel, const ChunkKey& key,
...
AbstractBuffer* FileMgr::putBuffer(const MemoryLevel memLevel, const ChunkKey& key,
  ...
  if (chunkIt == chunkIndex_.end()) {
    chunk = createBuffer(memLevel, key, defaultPageSize_);
  } else {
...
AbstractBuffer* FileMgr::alloc(const MemoryLevel memLevel, const size_t numBytes = 0) {
