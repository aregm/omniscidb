...
class GlobalFileMgr : public AbstractBufferMgr {  // implements
 ...
 public:
  ...
  AbstractBuffer* createBuffer(const MemoryLevel memLevel, const ChunkKey& key,
    ...
    return getFileMgr(key)->createBuffer(memLevel, key, pageSize, numBytes);
  }
  ...
  AbstractBuffer* getBuffer(const MemoryLevel memLevel, const ChunkKey& key, const size_t numBytes = 0) override {
    return getFileMgr(key)->getBuffer(memLevel, key, numBytes);
  }

  void fetchBuffer(const MemoryLevel memLevel, const ChunkKey& key,
                   AbstractBuffer* destBuffer,
                   const size_t numBytes) override {
    return getFileMgr(key)->fetchBuffer(memLevel, key, destBuffer, numBytes);
  }
  ...
  AbstractBuffer* putBuffer(const MemoryLevel memLevel, const ChunkKey& key,
                            AbstractBuffer* d,
                            const size_t numBytes = 0) override {
    return getFileMgr(key)->putBuffer(memLevel, key, d, numBytes);
  }

  // Buffer API
  AbstractBuffer* alloc(const MemoryLevel memLevel, const size_t numBytes) override {
