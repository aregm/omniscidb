...
AbstractBuffer* BufferMgr::createBuffer(const MemoryLevel memLevel, const ChunkKey& chunkKey,
  ...
  // so can't be evicted)
  try {
    allocateBuffer(memLevel, chunkIndex_[chunkKey], actualChunkPageSize, initialSize);
...
BufferList::iterator BufferMgr::reserveBuffer(
  ...
  auto newSegIt = findFreeBuffer(memLevel, numBytes);
...
BufferList::iterator BufferMgr::findFreeBuffer(const MemoryLevel memLevel, size_t numBytes) {
  size_t numPagesRequested = (numBytes + pageSize_ - 1) / pageSize_;
  ...
  while (!allocationsCapped_ && numPagesAllocated_ < maxNumPages_) {
    try {
      ...
      if (numPagesRequested <= currentMaxSlabPageSize_) {  // don't try to allocate if the
        ...
        auto alloc_ms =
            measure<>::execution([&]() { addSlab(memLevel, currentMaxSlabPageSize_ * pageSize_); });
      ...
      return findFreeBufferInSlab(
	    ...
	    printf("failed to allcoate slab\n");
...
void BufferMgr::checkpoint() {
  ...
  for (auto bufferIt = chunkIndex_.begin(); bufferIt != chunkIndex_.end(); ++bufferIt) {
    if (bufferIt->second->chunkKey[0] != -1 &&
      ...
      parentMgr_->putBuffer(bufferIt->second->buffer->memLevel_, bufferIt->second->chunkKey, bufferIt->second->buffer);
...
AbstractBuffer* BufferMgr::getBuffer(const MemoryLevel memLevel, const ChunkKey& key, const size_t numBytes) {
  std::lock_guard<std::mutex> lock(globalMutex_);  // granular lock
  ...
  if (foundBuffer) {
    ...
    if (bufferIt->second->buffer->size() <
      ...
      parentMgr_->fetchBuffer(memLevel, key, bufferIt->second->buffer, numBytes);
    }
    return bufferIt->second->buffer;
  } else {  // If wasn't in pool then we need to fetch it
    sizedSegsLock.unlock();
    AbstractBuffer* buffer =
        createBuffer(memLevel, key, pageSize_, numBytes);  // createChunk pins for us
    try {
      parentMgr_->fetchBuffer(
          memLevel, key, buffer, numBytes);  // this should put buffer in a BufferSegment
...
void BufferMgr::fetchBuffer(const MemoryLevel memLevel, const ChunkKey& key,
  ...
  if (!foundBuffer) {
    sizedSegsLock.unlock();
    CHECK(parentMgr_ != 0);
    buffer = createBuffer(memLevel, key, pageSize_, numBytes);  // will pin buffer
    try {
      parentMgr_->fetchBuffer(memLevel, key, buffer, numBytes);
    } catch (std::runtime_error& error) {
...
AbstractBuffer* BufferMgr::putBuffer(const MemoryLevel memLevel, const ChunkKey& key,
  ...
  AbstractBuffer* buffer;
  if (!foundBuffer) {
    buffer = createBuffer(memLevel, key, pageSize_);
  } else {
    buffer = bufferIt->second->buffer;
  }
...
AbstractBuffer* BufferMgr::alloc(const MemoryLevel memLevel, const size_t numBytes) {
  std::lock_guard<std::mutex> lock(globalMutex_);
  ChunkKey chunkKey = {-1, getBufferId()};
  printf("allocate %ld bytes\n", numBytes);
  return createBuffer(memLevel, chunkKey, pageSize_, numBytes);
}
