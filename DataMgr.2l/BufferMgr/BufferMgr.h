...
class BufferMgr : public AbstractBufferMgr {  // implements
 ...
 public:
  ...
  AbstractBuffer* createBuffer(const MemoryLevel memLevel, const ChunkKey& key,
  ...
  AbstractBuffer* getBuffer(const MemoryLevel memLevel, const ChunkKey& key, const size_t numBytes = 0) override;
  ...
  void fetchBuffer(const MemoryLevel memLevel, const ChunkKey& key,
                   AbstractBuffer* destBuffer,
                   const size_t numBytes = 0) override;
  AbstractBuffer* putBuffer(const MemoryLevel memLevel, const ChunkKey& key,
                            AbstractBuffer* d,
                            const size_t numBytes = 0) override;
  ...
  // Buffer API
  AbstractBuffer* alloc(const MemoryLevel memLevel, const size_t numBytes = 0) override;
  ...
  BufferList::iterator reserveBuffer(const MemoryLevel memLevel, BufferList::iterator& segIt, const size_t numBytes);
 ...
 private:
  ...
  int getBufferId();
  virtual void addSlab(const MemoryLevel memLevel, const size_t slabSize) = 0;
  virtual void freeAllMem() = 0;
  virtual void allocateBuffer(const MemoryLevel memLevel, BufferList::iterator segIt,
  ...
  BufferList::iterator evict(BufferList::iterator& evictStart,
                             const size_t numPagesRequested,
                             const int slabNum);
  BufferList::iterator findFreeBuffer(const MemoryLevel memLevel, size_t numBytes);
