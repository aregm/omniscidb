...
AbstractBuffer* DataMgr::createChunkBuffer(const ChunkKey& key,
  ...
  return bufferMgrs_[level][deviceId]->createBuffer(memoryLevel, key, page_size);
}
...
AbstractBuffer* DataMgr::getChunkBuffer(const ChunkKey& key,
  ...
  return bufferMgrs_[level][deviceId]->getBuffer(memoryLevel, key, numBytes);
}
...
AbstractBuffer* DataMgr::alloc(const MemoryLevel memLevel,
                               const int deviceId,
                               const size_t numBytes) {
  int level = static_cast<int>(memLevel);
  if (level == PMM_LEVEL) {
	  level = CPU_LEVEL;
  }

  assert(deviceId < levelSizes_[level]);
  return bufferMgrs_[level][deviceId]->alloc(memLevel, numBytes);
}
